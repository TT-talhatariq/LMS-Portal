...
/// For Breadcrumbs in Course Details Page
export const getCourseBreadCrumbs = async (courseId) => {
  try {
    const supabase = await createClient();
    const { data, error } = await supabase
      .from('courses')
    ...
    if (error) {
      console.error(`Supabase error fetching course ${courseId}:`, error);
      throw new Error('Database query failed');
    }

    if (!data || data.length === 0) {
      console.log("Cannot find Course Details for Course ID:", courseId);
      return [];
    }
